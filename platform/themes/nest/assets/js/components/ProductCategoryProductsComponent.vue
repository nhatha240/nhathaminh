            ...
            <div class="heading-tab-right wow fadeIn animated">
                <ul class="nav nav-tabs right no-border" role="tablist">
                    <li class="nav-item" role="presentation" v-for="item in productCategories" :key="item.id">
                        <button :class="productCategory.id === item.id ? 'nav-link active': 'nav-link'" data-bs-toggle="tab" :data-bs-target="'#' + item.slug" type="button" role="tab" :aria-controls="item.slug" aria-selected="true" @click="getData(item)" v-html="item.name">{</button>
                    </li>
                </ul>
            </div>
        </div>
