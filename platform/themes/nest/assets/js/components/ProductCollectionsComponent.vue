    ...
    <div class="container">
        <ul class="nav nav-tabs" role="tablist">
            <li class="nav-item" role="presentation" v-for="item in productCollections" :key="item.id">
                <button :class="productCollection.id === item.id ? 'nav-link active': 'nav-link'" data-bs-toggle="tab" :data-bs-target="'#' + item.slug" type="button" role="tab" :aria-controls="item.slug" aria-selected="true" @click="getData(item)" v-html="item.name"></button>
            </li>
        </ul>
        <!--End nav-tabs-->
